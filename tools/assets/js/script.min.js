    ...
    <div class="col">
      <div class="card shadow-sm h-100">
        <div class="card-body d-flex flex-column card-chinese">
          ...
          <p class="card-text mt-auto" style="padding-left:4rem">${buildIdiomCardContent(e)}</p>
        </div>
      </div>
    </div>`).join("")}function renderHomeIdioms(){var e=$("random-idioms");allIdioms.length&&renderIdiomCards(e,shuffleArrayInPlace([...allIdioms]).slice(0,3))}function renderRandomIdiomStories(){var e=$("search-results"),t=allIdioms.filter(e=>e.story?.length);if(!t.length)return renderStatusMessage(e,"暂无成语故事");renderIdiomCards(e,shuffleArrayInPlace(t).slice(0,3).map(e=>({...e,definition:(e.definition||"")+"<br /><strong>故事</strong> "+e.story.join("<br /><br />")})))}let searchPaginator=null;async function searchIdioms(e){if(!e)return[];let i=e.toLowerCase().split(/\s+/).filter(Boolean);return(await Promise.all(idiomsParts.map(e=>e.filter(t=>i.every(e=>t.idiomLower.includes(e)||t.definitionLower.includes(e)||t.litLower.includes(e)||t.figLower.includes(e)))))).flat()}async function handleIdiomSearch(){let n=$("search-input").value.trim();var e=n.toLowerCase();let i=$("search-results");var r=$("pagination-controls-dict"),a=$("button-addon2");if(i.innerHTML="",r&&(r.innerHTML=""),searchPaginator&&(searchPaginator.destroy(),searchPaginator=null),e.length<2)renderStatusMessage(i,"请输入至少2个字符"),e||renderRandomIdiomStories(),a.disabled=!0;else{a.disabled=!1;let t=await searchIdioms(e);if(!t.length)return renderStatusMessage(i,"未找到相关成语");r=Math.ceil(t.length/3);(searchPaginator=new Paginator("pagination-controls-dict",r,e=>{e=t.slice(3*(e-1),3*e).map(t=>{let i={idiom:t.idiom,definition:t.definition,lit:t.lit,fig:t.fig};return n.split(/\s+/).filter(Boolean).forEach(e=>{i.idiom=highlightText(i.idiom,e),i.definition=highlightText(i.definition||"",e)+(t.story?.length?"<br /><strong>故事</strong> "+t.story.join("<br /><br />"):""),i.lit=highlightText(i.lit||"",e),i.fig=highlightText(i.fig||"",e)}),{...t,...i}});renderIdiomCards(i,e)})).render(1)}}function renderIgcseIdioms(e=1){let t=$("igcse-idioms");var i,n,r="pagination-controls-igcse",a=(t.innerHTML="",$(r).innerHTML="",Object.entries(igcseData||{}).sort((e,t)=>t[0].localeCompare(e[0],"en",{numeric:!0})));let o=[];for([i,n]of a)for(var[s,l]of Object.entries(n)){s=idiomMap.get(s);s&&o.push({...s,exampleSentence:`${l} (${i})`})}if(!o.length)return t.innerHTML="<p>暂无真题成语数据</p>";a=Math.ceil(o.length/3);new Paginator(r,a,e=>renderIdiomCards(t,o.slice(3*(e-1),3*e))).render(e)}let currentQuestion=null,currentScore=0,bestScore=parseInt(localStorage.getItem("idiomGameBestScore"))||0;function renderNextGameQuestion(){var e=allIdioms[Math.floor(Math.random()*allIdioms.length)],t=shuffleArrayInPlace([...allIdioms]).slice(0,3).map(e=>e.idiom),i=(t.includes(e.idiom)||(t[Math.floor(3*Math.random())]=e.idiom),t.indexOf(e.idiom));currentQuestion={definition:e.definition,correctIndex:i},$("question").innerHTML=`<strong style="border-bottom:var(--text-color) 1px dashed">“${e.definition}” 对应的成语是？</strong>`,$("options").innerHTML=t.map((e,t)=>`
      <div class="col">
