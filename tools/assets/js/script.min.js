    ...
    <div class="col">
      <div class="card shadow-sm h-100">
        <div class="card-body d-flex flex-column card-chinese">
          ...
          <p class="card-text mt-auto" style="padding-left:4rem">${buildIdiomCardContent(e,i)}</p>
        </div>
      </div>
    </div>`).join("")}function renderHomeIdioms(){var e=$("random-idioms");allIdioms.length&&renderIdiomCards(e,shuffleArrayInPlace([...allIdioms]).slice(0,3),!1)}function renderRandomIdiomStories(){var e=$("search-results"),t=allIdioms.filter(e=>e.story?.length);if(!t.length)return renderStatusMessage(e,"暂无成语故事");renderIdiomCards(e,shuffleArrayInPlace(t).slice(0,3),!0)}let searchPaginator=null;async function searchIdioms(e){if(!e)return[];let n=e.toLowerCase().split(/\s+/).filter(Boolean);return(await Promise.all(idiomsParts.map(e=>e.map(e=>{let t=0;for(var i of n)(e.idiomLower.includes(i)||e.definitionLower.includes(i)||e.litLower.includes(i)||e.figLower.includes(i))&&t++;return 0<t?{...e,matchCount:t}:null}).filter(Boolean)))).flat().sort((e,t)=>t.matchCount-e.matchCount)}async function handleIdiomSearch(){let i=$("search-input").value.trim();var e=i.toLowerCase();let n=$("search-results");var a=$("pagination-controls-dict"),r=$("button-addon2");if(n.innerHTML="",a&&(a.innerHTML=""),searchPaginator&&(searchPaginator.destroy(),searchPaginator=null),e.length<2)renderStatusMessage(n,"请输入至少2个字符"),e||renderRandomIdiomStories(),r.disabled=!0;else{r.disabled=!1;let t=await searchIdioms(e);if(!t.length)return renderStatusMessage(n,"未找到相关成语");a=Math.ceil(t.length/2);(searchPaginator=new Paginator("pagination-controls-dict",a,e=>{e=t.slice(2*(e-1),2*e).map(e=>{let t={idiom:e.idiom,definition:e.definition,lit:e.lit,fig:e.fig};return i.split(/\s+/).filter(Boolean).forEach(e=>{t.idiom=highlightText(t.idiom,e),t.definition=highlightText(t.definition||"",e),t.lit=highlightText(t.lit||"",e),t.fig=highlightText(t.fig||"",e)}),{...e,...t}});renderIdiomCards(n,e,!0)})).render(1)}}function renderIgcseIdioms(e=1){let t=$("igcse-idioms");var i,n,a="pagination-controls-igcse",r=(t.innerHTML="",$(a).innerHTML="",Object.entries(igcseData||{}).sort((e,t)=>t[0].localeCompare(e[0],"en",{numeric:!0})));let o=[];for([i,n]of r)for(var[s,l]of Object.entries(n)){s=idiomMap.get(s);s&&o.push({...s,exampleSentence:`${l} (${i})`})}if(!o.length)return t.innerHTML="<p>暂无真题成语数据</p>";r=Math.ceil(o.length/2);new Paginator(a,r,e=>renderIdiomCards(t,o.slice(2*(e-1),2*e),!1)).render(e)}let currentQuestion=null,currentScore=0,bestScore=parseInt(localStorage.getItem("idiomGameBestScore"))||0;function renderNextGameQuestion(){var e=allIdioms[Math.floor(Math.random()*allIdioms.length)],t=shuffleArrayInPlace([...allIdioms]).slice(0,3).map(e=>e.idiom),i=(t.includes(e.idiom)||(t[Math.floor(3*Math.random())]=e.idiom),t.indexOf(e.idiom));currentQuestion={definition:e.definition,correctIndex:i},$("question").innerHTML=`<strong style="border-bottom:var(--text-color) 1px dashed">“${e.definition}” 对应的成语是？</strong>`,$("options").innerHTML=t.map((e,t)=>`
      <div class="col">
